...
MixEHR is a Unix-style command-line tool. You can compile it on a unix machine. I included 3 Makefiles. The default linked to the one work on my MacBook Pro (MacOS 10.13.4)

Assuming you are in the mixehr directory, to compile, simply run:

make

If you need to switch Makefile, please link to any of the 3 Makefiles in the 'Makefiles' folder.

I also included ‘macos_version’ and ‘ubuntu_version’ binary program, in case the first one does not work for you.

If it complains ‘armadillo’ related issues, try add the ‘lib64’ to your LD_LIBRARY_PATH environment before running it. e.g.,

LD_LIBRARY_PATH="~/util/armadillo8/usr/lib64:$LD_LIBRARY_PATH"
...
I included the MIMIC-III data used for my NIPS 2018 paper. To run mixehr, you will need a meta data file that contains 3 columns: 
...
See the example file ‘mimic_meta.txt’ in the folder.

The actual EHR data file (mimic_trainData.txt) has 5 columns rows:
