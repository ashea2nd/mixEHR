...
# ROC auc
auroc <- function(myscores, labels_true) {
  
  unlist(slot(
    performance( prediction(myscores, labels_true), "auc" ), 
    "y.values"))
}
...
# PR auc
auprc <- function(myscores, labels_true) {
  ...
  integrate(approxfun(cbind(rec, prec)), lower=0, upper=1,
...
prceval <- function(myscores, labels_true, model) {
  ...
  prcdf <- data.frame(x=rec, y=prec, auc=auc,
                      evalTypeAUC=sprintf("%s (%s)", model, percent(auc)), 
                      model=model, curveType="PRC")
  
  return(prcdf)
}

# F1-score
f1score <- function(myscores, labels_true) {
  ...
  mean(2*(rec*prec)/(rec+prec), na.rm=TRUE)
}


# accuracy
acceval <- function(prelab, trulab) {
  
  sum(prelab == trulab)/length(trulab)
}
